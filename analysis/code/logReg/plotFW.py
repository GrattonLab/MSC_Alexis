...
def feature_plots(coef, f='DS', task='task', sub='i'):
    ...
    vmin=np.amin(ds)
    vmax=np.amax(ds)
    fig = figure_corrmat(ds,Parcel_params, clims=(-.01,.01))
    fig.savefig("/Users/Alexis/Desktop/MSC_Alexis/analysis/output/images/logReg/fw/" +f+ "/" +task+ "_" +sub+ ".png", bbox_inches='tight')
...
def saveFW(coef, f, task, sub):
    ...
    df.to_csv("/Users/Alexis/Desktop/MSC_Alexis/analysis/output/results/logReg/fw/" +f+ "/" +task+ "_" +sub+ ".csv")
...
def fwFolds(folds, f, task, sub):
    ...
    vmin=np.amin(ds)
    vmax=np.amax(ds)
    fig = figure_corrmat(ds,Parcel_params, clims=(0, .001))
    fig.savefig("/Users/Alexis/Desktop/MSC_Alexis/analysis/output/images/logReg/fw/" +f+ "/fwSD_Folds/" +task+ "_" +sub+ ".png", bbox_inches='tight')
...
#coef = svm.coef_
#feature_plots(coef,'DS','test','sub')
