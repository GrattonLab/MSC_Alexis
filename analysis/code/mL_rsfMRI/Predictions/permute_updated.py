...
def classifyDS():
    ...
    """
    ##fix this so the mean is for each task
    clf=RidgeClassifier()
    DS=pd.DataFrame()
...
def folds(clf,taskFC, restFC, test_taskFC, test_restFC):
    ...
    for train_index, test_index in loo.split(taskFC):
        ...
        clf.fit(X_tr,y_tr)
        SSscores=clf.score(Xtest,ytest)
...
def modelAll_binary(clf,train_sub, test_sub):
    ...
    restFC=npmatFiles(restFC,(10,4,55278)) #reshape to gather correct days
    #test sub
...
def foldsBinary(train_sub, clf, memFC,semFC,glassFC,motFC,restFC, test_taskFC, test_restFC):
    ...
    for train_index, test_index in loo.split(split):
        ...
        Xtrain_rest=npmatFiles(Xtrain_rest,(-1,55278))
        Xval_rest=npmatFiles(Xval_rest,(-1,55278))
...
def multiclassAll(classifier='Ridge'):
    ...
    #train sub
        ...
        #restFC=npmatFiles(restFC,(10,4,nsize)) #reshape to gather correct days
        #test sub
...
def permut_single_task():
    ...
    for i in range(1000):
        tmpdf=classifyDS()
        final=pd.concat([final,tmpdf])
        print(str(i))
    final.to_csv('/Users/Alexis/Desktop/MSC_Alexis/analysis/output/results/permutation/DS/ind_permutation.csv',index=False)
