function [ results_struct ] = svm_scripts_beta( rmat, labels, numFeatures, testRmat, testLabels, option)
%Last Updated: 02.12.19 Ashley Nielsen
...
if size(labels,2) > 1 % Multi-class SVM
...
%% SVM
    case 1  
        ...
        for i = 1:numSubjects
            ...
            clear trainData tr a testData tst tempLabels tempRmat testLabel testFeatures
        end
        ...
        if option == 2 % If you do not want to do LOOCV
            ...
            for f = 1:length(numFeatures)
                ...
                for t = 1:length(testLabels)
                    ...
                    testsData = data(testsFeatures,testLabels(t));
            ...
            predictedLabels(:,f) = hits;
            hitRate = sum(hits)./length(testLabels).*100;
            performanceMeasure = hitRate;
    ...
    case 4  
        ...
        if option == 2
            ...
            for f = 1:length(numFeatures)
                ...
                testsFeatures = zeros(1,nonZFeatures);
...
%                     end
                    ...
                    clear testsFeatures testsData
                %end
                featureWeights(1:numFeatures(f),f) = get_w(a);
            end
            performanceMeasure = corr(testLabels,predictedLabels).^2;
            performanceName = 'R2';
        ...
        if option == 2
            ...
            for f = 1:length(numFeatures)
                ...
                for t = 1:size(testLabels,1)
                    for j = 1:numFeatures(f)
                        testsFeatures(j) = testRmat(featureList(j,1,1,f),featureList(j,2,1,f),t);
                    end
                    ...
                    predictedLabels(t,f) = find(testsTST.X==1);
                    clear testsFeatures testsData
                end
